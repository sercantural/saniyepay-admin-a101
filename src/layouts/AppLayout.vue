  ...
  const groups = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Merchants', title: 'Bayiler', sub: 'Bayi ve API', icon: 'mdi-store-outline', show: auth.isSuperAdmin },
        { name: 'Reports', title: 'Raporlar', sub: 'Finansal raporlar', icon: 'mdi-chart-bar', show: can('reports.view') },
        { name: 'ApiLogs', title: 'API Kayıtları', sub: 'İstek ve yanıtlar', icon: 'mdi-console-network', show: auth.isSuperAdmin },
        { name: 'Proposals', title: 'Teklifler', sub: 'Bayi teklifleri', icon: 'mdi-file-document-outline', show: auth.isSuperAdmin },
        { name: 'ClockTracking', title: 'Mesai Takibi', sub: 'Personel kayıtları', icon: 'mdi-clock-check-outline',
          show: auth.isSuperAdmin || auth.user?.roles?.some((r) => ['grup_yoneticisi', 'sub_group_manager'].includes(r.name)) },
        { name: 'PlatformSettings', title: 'Platform Ayarları', sub: 'Sistem yapılandırması', icon: 'mdi-cog-outline', show: auth.isSuperAdmin },
      ],
    },
  ]
  return groups
